frappe.ui.form.on('OpenWA Settings', {
	refresh(frm) {
		frm.add_custom_button(__('Refresh Status & QR'), function() {
			frappe.call({
				method: 'gravures_custom.gravures_custom.doctype.openwa_settings.openwa_settings.get_session_status',
				callback: function(r) {
					if (r.message) {
		...
		frm.add_custom_button(__('Start Session'), function() {
			frappe.call({
				method: 'gravures_custom.gravures_custom.doctype.openwa_settings.openwa_settings.start_session',
				callback: function(r) {
					if (r.message) {
		...
		frm.add_custom_button(__('Stop Session'), function() {
			frappe.call({
				method: 'gravures_custom.gravures_custom.doctype.openwa_settings.openwa_settings.stop_session',
				callback: function(r) {
					if (r.message) {
						frappe.show_alert({message: r.message.message, indicator: r.message.status === 'ok' ? 'green' : 'red'});
						frm.reload();
		...
		frm.add_custom_button(__('Create New Session'), function() {
			frappe.confirm(__('This will create a new session and invalidate the current one. Continue?'), function() {
				frappe.call({
					method: 'gravures_custom.gravures_custom.doctype.openwa_settings.openwa_settings.create_new_session',
					callback: function(r) {
						if (r.message) {
							frappe.show_alert({message: r.message.message, indicator: r.message.status === 'ok' ? 'green' : 'red'});
							frm.reload();
		...
		frm.add_custom_button(__('Get QR Code'), function() {
			frappe.call({
				method: 'gravures_custom.gravures_custom.doctype.openwa_settings.openwa_settings.get_session_qr',
				callback: function(r) {
