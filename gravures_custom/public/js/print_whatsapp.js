...
// Hooks PrintView via prototype patching and injects via DOM polling.

try {
    window.__gc_print_whatsapp_loaded = Date.now();
    console.log("[GC] print_whatsapp.js bootstrap start");

    (function () {
        "use strict";
        console.log("[GC] IIFE entered");
        ...
        function sendToWhatsApp(btn) {
            try {
                ...
                try {
                    const $p = $("select[data-fieldname='print_format']");
                    if ($p.length) print_format = $p.val() || null;
                } catch (e) {}
                try {
                    const $l = $("select[data-fieldname='language']");
                    if ($l.length) _lang = $l.val() || null;
                } catch (e) {}
        ...
        function injectIntoToolbar() {
            try {
                ...
                console.log("[GC] button injected");
                return true;
            } catch (e) {
                console.error("[GC] inject error:", e);
                return false;
            }
        }
            ...
            try {
                ...
                const waitForRouter = () => {
                    if (window.frappe && frappe.router && typeof frappe.router.on === "function") {
                        console.log("[GC] router found, attaching listener");
                        frappe.router.on("change", () => {
                            console.log("[GC] route changed, re-injecting");
                ...
                console.error("[GC] startInjector error:", e);
            }
        }

        startInjector();
        console.log("[GC] IIFE done");
    })();
} catch (e) {
    console.error("[GC] OUTER error:", e);
}
